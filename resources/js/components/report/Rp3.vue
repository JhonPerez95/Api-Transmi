          ...
          <div class="row">
            ...
            <div class="form-group col">
              <b-form-group label="Cédula" label-for="biker_document-input">
                ...
                v-slot="{ errors }"
              >
                <v-select multiple v-model="form.biker_document" :options="bikersData" :reduce="biker => biker.document" label="fullName"/>
...
export default {
  ...
  methods: {
    dataSubmit(event) {
      ...
      this.$api.get(`/web/data/reports/visits/detailedBikerByMonths?begining_date=${date_input}&end_date=${date_output}${bikerDocument}`).then((res) => {
          if (res.status == 200) {
    ...
    getData(){
        ...
        if(res.status == 200){
            this.bikersData = res.data.response.users.map(el => { el.fullName = `${el.document} - ${el.name} ${el.last_name}`;return el;} );
        }
      });
    }
  },
